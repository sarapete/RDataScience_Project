...
####Zip Code Density
Zoomed-in map of migration to Buffalo from Puerto Rico along great circle lines, weighted by the number from each place (the darker red the color, the more people coming from the zip code)
```{r}
...
Create a choropleth map of density (number of people coming to Buffalo from each Puerto Rican zip code)
```{r}
...
There are other ways of visualizing this data. For example, the following map will create centroid dots of the zip code of origin, with size indicating the number of individuals coming from that place
```{r, message=F}
